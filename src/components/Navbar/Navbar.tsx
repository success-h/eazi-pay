...
export function Navbar() {
  ...
  useEffect(() => {
    AOS.init({ duration: 500, easing: "ease-out-cubic", offset: -100 });
    AOS.refresh();
    if (window) {
  ...
  return (
    <nav
      className={clsx(`fixed top-0 w-full h-20 z-10 px-5 items-center`, {
        ["block"]: showNavbar,
        "lg:hidden": !showNavbar,
        "backdrop-blur-lg": !isOpen,
      })}
    >
      <div className="flex z-10 py-10 justify-between items-center mx-auto w-full max-w-screen-xl h-full">
      ...
      >
        <div className="relative gap-7 cursor-pointer l:hidden">
          <Link href="#" className="btn-ghost btn btn-block justify-normal">
            About us
          </Link>
